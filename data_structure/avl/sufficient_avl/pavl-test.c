...
static void
print_tree_structure (const struct pavl_node *node, int level)
{
  ...
  printf ("%d", *(int *) node->pavl_data);
  if (node->pavl_link[0] != NULL || node->pavl_link[1] != NULL)
    {
      putchar ('(');

      print_tree_structure (node->pavl_link[0], level + 1);
      if (node->pavl_link[1] != NULL)
        {
          putchar (',');
          print_tree_structure (node->pavl_link[1], level + 1);
        }
...
void
print_whole_tree (const struct pavl_table *tree, const char *title)
{
  printf ("%s: ", title);
  print_tree_structure (tree->pavl_root, 0);
  putchar ('\n');
}
...
static int
check_traverser (struct pavl_traverser *trav, int i, int n, const char *label)
{
  int okay = 1;
  int *cur, *prev, *next;

  prev = pavl_t_prev (trav);
  if ((i == 0 && prev != NULL)
...
static int
verify_tree (struct pavl_table *tree, int array[], size_t n)
{
  int okay = 1;

  /* Check |tree|'s bst_count against that supplied. */
  if (pavl_count (tree) != n)
    {
      printf (" Tree count is %lu, but should be %lu.\n",
              (unsigned long) pavl_count (tree), (unsigned long) n);
      okay = 0;
    }
  ...
  if (okay)
    {
      ...
      recurse_verify_tree (tree->pavl_root, &okay, &count,
                           0, INT_MAX, &height);
      if (count != n)
        {
  ...
  if (okay)
    {
      ...
      for (i = 0; i < n; i++)
        if (pavl_find (tree, &array[i]) == NULL)
          {
  ...
  if (okay)
    {
      /* Check that |pavl_t_first()| and |pavl_t_next()| work properly. */
      struct pavl_traverser trav;
      size_t i;
      int prev = -1;
      int *item;

      for (i = 0, item = pavl_t_first (&trav, tree); i < 2 * n && item != NULL;
           i++, item = pavl_t_next (&trav))
        {
          if (*item <= prev)
            {
  ...
  if (okay)
    {
      /* Check that |pavl_t_last()| and |pavl_t_prev()| work properly. */
      struct pavl_traverser trav;
      size_t i;
      int next = INT_MAX;
      int *item;

      for (i = 0, item = pavl_t_last (&trav, tree); i < 2 * n && item != NULL;
           i++, item = pavl_t_prev (&trav))
        {
          if (*item >= next)
            {
  ...
  if (okay)
    {
      /* Check that |pavl_t_init()| works properly. */
      struct pavl_traverser init, first, last;
      int *cur, *prev, *next;

      pavl_t_init (&init, tree);
      pavl_t_first (&first, tree);
      pavl_t_last (&last, tree);
      ...
      next = pavl_t_next (&init);
      if (next != pavl_t_cur (&first))
        {
          printf (" Next after null should be %d, but is actually %d.\n",
                  *(int *) pavl_t_cur (&first), *next);
          okay = 0;
        }
      pavl_t_prev (&init);

      prev = pavl_t_prev (&init);
      if (prev != pavl_t_cur (&last))
        {
          printf (" Previous before null should be %d, but is actually %d.\n",
                  *(int *) pavl_t_cur (&last), *prev);
          okay = 0;
        }
      pavl_t_next (&init);
    }
                  ...
                  int insert[], int delete[], int n, int verbosity)
{
  ...
  /* Test creating a PAVL and inserting into it. */
  tree = pavl_create (compare_ints, NULL, allocator);
  if (tree == NULL)
    {
      if (verbosity >= 0)
  ...
  for (i = 0; i < n; i++)
    {
      ...
      {
        void **p = pavl_probe (tree, &insert[i]);
        if (p == NULL)
          {
            if (verbosity >= 0)
              printf ("    Out of memory in insertion.\n");
            pavl_destroy (tree, NULL);
            return 1;
          }
        if (*p != &insert[i])
  ...
  /* Test PAVL traversal during modifications. */
  for (i = 0; i < n; i++)
    {
      struct pavl_traverser x, y, z;
      int *deleted;
      ...
      if (pavl_t_find (&x, tree, &insert[i]) == NULL)
        {
      ...
      deleted = pavl_delete (tree, &delete[i]);
      ...
      if (pavl_t_insert (&z, tree, &delete[i]) == NULL)
        {
          if (verbosity >= 0)
  ...
  for (i = 0; i < n; i++)
    {
      ...
      {
        struct pavl_table *copy = pavl_copy (tree, NULL, NULL, NULL);
        if (copy == NULL)
          {
            if (verbosity >= 0)
        ...
        okay &= compare_trees (tree->pavl_root, copy->pavl_root);
        pavl_destroy (copy, NULL);
      }
    }

  if (pavl_delete (tree, &insert[0]) != NULL)
    {
...

static int
test_bst_t_first (struct pavl_table *tree, int n)
{
  struct pavl_traverser trav;
  int *first;

  first = pavl_t_first (&trav, tree);
...
static int
test_bst_t_last (struct pavl_table *tree, int n)
{
  struct pavl_traverser trav;
  int *last;

  last = pavl_t_last (&trav, tree);
...
static int
test_bst_t_find (struct pavl_table *tree, int n)
{
  int i;
...
static int
test_bst_t_insert (struct pavl_table *tree, int n)
{
  int i;
  ...
  for (i = 0; i < n; i++)
    {
      ...
      iter = pavl_t_insert (&trav, tree, &i);
...
static int
test_bst_t_next (struct pavl_table *tree, int n)
{
  struct pavl_traverser trav;
  int i;
...
static int
test_bst_t_prev (struct pavl_table *tree, int n)
{
  struct pavl_traverser trav;
  int i;
...
static int
test_bst_copy (struct pavl_table *tree, int n)
{
  struct pavl_table *copy = pavl_copy (tree, NULL, NULL, NULL);
  int okay = compare_trees (tree->pavl_root, copy->pavl_root);
...
/* Tests the tree routines for proper handling of overflows.
   ...
   with height greater than |PAVL_MAX_HEIGHT|.
   Uses |allocator| as the allocator for tree and node data.
               ...
               int order[], int n, int verbosity)
{
  ...
  typedef int test_func (struct pavl_table *, int n);
  ...
  for (i = test; i < test + sizeof test / sizeof *test; i++)
    {
      ...
      for (j = 0; j < n; j++)
        {
          void **p = pavl_probe (tree, &order[j]);
          if (p == NULL || *p != &order[j])
            {
